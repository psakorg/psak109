        ...
        <div class="container mt-5" style="padding-right: 50px;">
            <section class="section">
                <div class="section-header">
                    <h4>REPORT AMORTISED COST - SIMPLE INTEREST</h4>
                </div>
                @if(session('pesan'))
                    ...
                    <table class="table table-striped table-bordered custom-table" style="width: 100%; margin: 0 auto;">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 20%;">Outstanding Interest</th>
                                <th style="width: 25%;">Up Front Fee</th>
                        ...
                        <tbody>
                            @foreach ($loans as $loan)
                                <tr>
                                    ...
                                    <td>{{ number_format($loan->bilint, 2) }}</td>
                                    <td>{{ number_format($loan->prov, 2) }}</td>
                                    <td>{{ number_format($loan->trxcost, 2) }}</td>
                                    {{-- <td>{{ number_format(0, 2) }}</td> --}}
                                    <td>{{ number_format($loan->eirex*100, 14)}}%</td>
                                    <td>{{ number_format($loan->eircalc*100, 14)}}%</td>
                                    <td>
                                        <a href="{{ route('report-amorcost-si.view',  ['no_acc' => $loan->no_acc, 'id_pt' => $loan->id_pt]) }}" class="btn btn-sm btn-info">
