        ...
        <div class="container ms-5 mt-5">
            ...
            <div class="card">
                <div class="card-header">
                    <h6 class="m-0 font-weight-bold text-primary">tbldatasecurites</h6>
                </div>
                        ...
                        <tbody>
                            ...
                            @foreach($securities as $item)
                                ...
                                <tr>
                                    ...
                                    <td>{{ $item->mtr_date ? date('d/m/Y', strtotime($item->mtr_date)) : '' }}</td>
                                    <td>{{ $item->bond_jns }}</td>
                                    <td>{{ $item->transac_jns }}</td>
                                    <td>{{ $item->transac_dt ? date('d/m/Y', strtotime($item->transac_dt)) : '' }}</td>
                                    <td>{{ number_format($item->principal_out, 0) }}</td>
                                    <td>{{ number_format($item->principal_in, 0) }}</td>
                                    <td>{{ number_format($item->face_value, 0) }}</td>
                                    <td>{{ $item->eval_dt ? date('d/m/Y', strtotime($item->eval_dt)) : '' }}</td>
                                    <td>{{ $item->sale_dt ? date('d/m/Y', strtotime($item->sale_dt)) : '' }}</td>
                                    <td>{{ $item->change_dt ? date('d/m/Y', strtotime($item->change_dt)) : '' }}</td>
                                    <td>{{ $item->status_psak }}</td>
                                    <td>{{ $item->id_pt }}</td>
                                </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('upload.securities.data.import') }}" method="POST" enctype="multipart/form-data">
                        @csrf
