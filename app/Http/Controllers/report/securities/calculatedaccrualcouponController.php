    ...
    public function view($no_acc, $id_pt)
{
    ...
    public function exportExcel($no_acc,$id_pt)
    {
        ...
        $sheet->setCellValue('K11', 'Outstanding Amount');
        $sheet->getStyle('K11')->getFont()->setBold(true); // Set bold untuk Outstanding Amount
        $sheet->setCellValue('N11', $loan->no_acc);
        $sheet->setCellValue('K12', 'EIR Calculated Convertion');
        $sheet->getStyle('K12')->getFont()->setBold(true); // Set bold untuk EIR Calculated Convertion
        $sheet->setCellValue('N12', $loan->deb_name);

        // Set judul tabel laporan
        $sheet->setCellValue('A10', 'Report Calculated Accrual Coupon - Treasury Bonds');
