...
class evaluationTreasuryController extends Controller
{
    ...
    public function exportExcel(Request $request, $id_pt)
    {
        ...
        // Get the collection from your custom method (same as index)
        $loans = report_securities::callEvaluationTreasuryBonds(
            intval($tahun),
            intval($bulan),
            intval($hari),
        ...
        foreach ($loans as $key => $loan) {
            ...
            $sheet->setCellValue('D' . $row, $loan->bond_id ?? '');
            $sheet->setCellValue('E' . $row, $loan->issuer_name ?? '');
            $sheet->setCellValue('F' . $row, $loan->coa ?? '');
            $sheet->setCellValue('G' . $row, $loan->bond_type ?? '');
            $sheet->setCellValue('H' . $row, $loan->gl_group ?? '');
            $sheet->setCellValue('I' . $row, $loan->transac_dt ? date('d/m/Y', strtotime($loan->transac_dt)) : '');
            $sheet->setCellValue('J' . $row, $loan->transac_dt ? date('d/m/Y', strtotime($loan->transac_dt)) : '');
            $sheet->setCellValue('K' . $row, number_format(0, 5) . '%');
            $sheet->setCellValue('L' . $row, number_format((float) str_replace(['$', ','], '', $loan->face_value)));
            $sheet->setCellValue('M' . $row, number_format($loan->price * 100, 5));
            $sheet->setCellValue('N' . $row, number_format($loan->mtm_price, 5));
            $sheet->setCellValue('O' . $row, number_format($loan->carrying_amount, 5));
            $sheet->setCellValue('P' . $row, number_format($loan->gain_losses, 5));
            $sheet->setCellValue('Q' . $row, number_format($loan->cum_gain_losses, 5));
            
            // Right align numeric columns
            $sheet->getStyle('K' . $row . ':Q' . $row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
        ...
        $filename = 'EvaluationTreasuryBonds_' . date('Ymd') . '.xlsx';
