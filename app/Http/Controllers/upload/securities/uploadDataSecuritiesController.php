...
class uploadDataSecuritiesController extends Controller
{
    public function index(Request $request)
    {
        ...
        $securities = DB::table('securities.tbldatasecurities')
            ->where('id_pt', $id_pt)
            ->paginate($perPage);
        ...
        try {
            ...
            foreach ($rows as $index => $row) {
                ...
                try {
                    ...
                    $data = [
                        'idtrx' => trim((string)$row[0]),
                        'no_branch' => trim((string)$row[1]),
                        'no_acc' => trim((string)$row[2]),
                        'bond_id' => trim((string)$row[3]),
                        'status' => (int)$row[4],
                        'issuer_name' => trim((string)$row[5]),
                        'org_bal' => (float)str_replace(['$', ','], '', $row[6]),
                        'trade_dt' => $convertDate($row[7]),
                        'settle_dt' => $convertDate($row[8]),
                        'coupon_rate' => (float)str_replace(['$', ','], '', $row[9]),
                        'tenor' => (float)$row[10],
                        'mtr_date' => $convertDate($row[11]),
                        'bond_jns' => trim((string)$row[12]),
                        'transac_jns' => trim((string)$row[13]),
                        'transac_dt' => $convertDate($row[14]),
                        'principal_out' => (float)str_replace(['$', ','], '', $row[15]),
                        'principal_in' => (float)str_replace(['$', ','], '', $row[16]),
                        'face_value' => (float)str_replace(['$', ','], '', $row[17]),
                        'eval_dt' => $convertDate($row[18]),
                        'sale_dt' => $convertDate($row[19]),
                        'change_dt' => $convertDate($row[20]),
                        'status_psak' => (int)$row[21],
                        'id_pt' => $id_pt,
                    ];
                    ...
                    DB::table('securities.tbldatasecurities')->insert($data);
                    DB::commit();
                    $successCount++;
